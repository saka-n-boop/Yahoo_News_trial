...
2. YahooシートのE-I列が空欄の行に対し、本文、コメント数、Gemini分析を実行し、E-I列を更新。
...
# --- Gemini クライアントの初期化 ---
# ★GOOGLE_API_KEYはgenai.Client()がデフォルトで参照
try:
        ...
        response = GEMINI_CLIENT.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                ...
                response_schema={"type": "object", "properties": {
                    ...
                    "category": {"type": "string", "description": "経済、政治、技術、エンタメ、社会のいずれか"}, 
...
def fetch_article_body_and_comments(base_url: str) -> Tuple[str, int]:
    ...
    try:
        ...
        count_element = soup.find("span", class_=re.compile("CommentCount"))
        if count_element:
            text = count_element.get_text(strip=True).replace(",", "")
            if text.isdigit():
...
def write_and_sort_news_list_to_source(gc: gspread.Client, articles: list[dict]):
    ...
    if new_data:
        ...
        worksheet.update(f'A1:{range_end}', full_data_to_write, value_input_option='USER_ENTERED')
...
def process_and_update_yahoo_sheet(gc: gspread.Client):
    """ ③空欄のE/F列に本文とコメント数を取得し、④空欄のG～I列にGeminiの分析結果を記載する """
    ...
    for idx, data_row in enumerate(data_rows):
        ...
        print(f"  - 行 {row_num} (記事: {title[:20]}...): 詳細取得と分析を実行中...")
        
        try:
            # 1. 本文とコメント数を取得 (E, F列)
            article_body, comment_count = fetch_article_body_and_comments(url)
            
            # 2. Geminiで分析を実行 (G, H, I列)
            sentiment, category, relevance = analyze_with_gemini(article_body)
            
            # 3. 更新データを作成: [本文, コメント数, ポジネガ分類, カテゴリ分類, 関連度]
            new_data = [article_body, comment_count, sentiment, category, relevance]
            updates_dict[row_num] = new_data
            
        except Exception as e:
            print(f"  - 行 {row_num} の処理中にエラー: {e}")
            updates_dict[row_num] = ["取得エラー", 0, "ERROR", "ERROR", "0"]
...
def transfer_to_today_sheet(gc: gspread.Client):
    ...
    if final_data_to_write:
        ...
        ws_dest.update(f'A1:{range_end}', final_data_to_write, value_input_option='USER_ENTERED')
